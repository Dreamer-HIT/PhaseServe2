...
目标是让 1P1D + LLaMA2-7B 成为可重复 smoke test。
...
### Phase 1：1P1D 小模型可控验证

目标是验证算法方向，而不是追求最终大模型数字。

配置：

- 模型：LLaMA2-7B，必要时补 OPT-6.7B/OPT-13B 作为稳定性对照。
...
2. 生成 ShareGPT-like 和 LongBench-like 的可控小数据集，先在 1P1D + LLaMA2-7B 上跑通。
