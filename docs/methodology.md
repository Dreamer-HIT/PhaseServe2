...
> PhaseServe 是一个 pressure-budgeted phase scheduling system for phase-disaggregated LLM serving。它证明了，在完成 prefill/decode 静态资源解耦之后，剩余关键瓶颈不只是阶段内调度不匹配，而是 decode 侧队列、KV 和 swap 压力会通过 bridge 反向约束 prefill。PhaseServe 将这些运行时压力持续转化为 admission budget，并在该预算下执行 prefill batch shaping 与 KV-constrained decode active-set shaping，从而改善 workload 的瓶颈指标，例如 TTFT queueing、TPOT tail、SLO goodput 或不同长度 bucket 的公平性。
...
2. 它把局部 prefill/decode scheduling 放到一个 pressure-to-budget 控制框架下，避免论文看起来只是两个 scheduler heuristic 加一个 watermark。
3. 它能做干净消融：pressure-budget controller、prefill shaping、decode active-set shaping 可以分别打开和关闭。

## 最终论文算法呈现

最终论文建议呈现 **3 个算法**，但不要把它们写成三个平权并列的算法。正确组织是：一个核心 pressure-budget controller，加两个 budgeted execution policies。

这 3 个算法不是为了声称“所有指标同时提升”，而是分别回答 pressure-budgeted phase scheduling 的三个问题：

| 算法 | 回答的问题 | 主要优化指标 | 允许的 tradeoff |
|---|---|---|---|
| Algorithm 1: PBC, Pressure-Budget Controller | 如何把 decode-side queue/KV/swap pressure 转成 prefill/decode admission budget | SLO goodput、per-GPU goodput、bridge queue、swap pressure | 可能降低 prefill aggressiveness，以避免 downstream decode overload |
...
| Algorithm 3: KAS, KV-Constrained Attained-Service Scheduling | 在 KV/swap budget 下如何选择 decode active set | TPOT P90/P99、decode queue time、short-output slowdown | 长输出请求平均完成时间可能轻微上升，需要 fairness bound |

正文中的呈现方式建议是：

1. 先给出一个统一的 **pressure-budget model**，定义 observable pressure、admission budget 和 workload bottleneck metric。
2. 再给出核心算法 PBC：将 decode-side pressure 转成 prefill budget、decode budget 和 safety margin。
3. 最后给出两个 budgeted execution policies：BPS 在 prefill 侧执行 batch shaping，KAS 在 decode 侧执行 active-set shaping。

不要把论文写成“我们提出了三个独立调度启发式”。更好的叙事是：

> PhaseServe converts runtime pressure into admission budgets, then uses budgeted prefill and decode shaping policies to optimize the workload bottleneck while bounding fairness and scheduling overhead.

中文表述：

> PhaseServe 将运行时压力转化为 admission budget，再用受预算约束的 prefill/decode shaping 策略优化 workload 瓶颈，同时约束公平性和调度开销。
...
因此，一个好的 disaggregated serving runtime 不应该对两个阶段都使用同一种 FCFS-like 策略，也不应该让 prefill 和 decode 完全各自为政，而应该使用 pressure-budgeted phase scheduling：
...
为了让方法论达到顶会论文的论证强度，PhaseServe 不应只描述为“两个启发式调度器”，而应被建模为一个 pressure-budgeted online control problem：PBC 根据下游压力生成 budget，BPS 和 KAS 分别在两个信息结构不同的阶段内执行预算约束下的在线调度。
...
> 在 prefill-decode disaggregated serving 中，阶段间资源隔离之后的剩余瓶颈主要来自阶段内信息不匹配和阶段间压力传播。将 decode-side queue/KV/swap pressure 转化为 admission budget，并在该 budget 下分别执行 known-size prefill shaping 与 unknown-size KV-constrained decode shaping，可以在异构 workload 下改善对应瓶颈指标和 SLO goodput。

这个假设有明确失败条件：如果 workload 的 prompt length 和 output length 都高度同质，或者系统远低于饱和，PhaseServe 的收益应显著下降；如果调度开销或 swap 开销超过收益，则方法不成立。

## 方法一：BPS：Budgeted Prefill Shaping

说明：为了便于解释，本文档先介绍两个局部执行策略，再介绍控制器；最终论文正文建议按 PBC、BPS、KAS 的顺序呈现。BPS 是旧版 `PS-Prefill` 的收敛版本，它不再直接声称“pressure-aware”，而是显式接受 PBC 给出的 admission budget。
...
7. `AdmissionBudget`：来自 PBC 的预算对象；第一版可以只接入 `prefill_token_budget` 和 `prefill_block_margin`。
...
如果 PBC 判断 decode pressure 较高，`prefill_block_margin` 增大，候选 batch 的 token budget 和 KV footprint 更保守；如果 decode pressure 较低，budget 放宽，prefill 可以更积极地填满 batch。
...
第三个不变量很重要。它能避免 reviewer 质疑 PhaseServe 通过人为延迟请求来换取更好 batch composition。BPS 只重排已经到达的 bounded window。
...
BPS 的 starvation bound 不需要给出强理论最优性，但要能给出工程上可验证的上界：
...
如果这些指标显示长请求 tail latency 爆炸，说明 BPS 的参数需要调整，不能只报告短请求收益。
...
5. `AdmissionBudget` 可以先只接入静态预算，等 PBC 实现后再动态调节 safety margin。
...
## 方法二：KAS：KV-Constrained Attained-Service Scheduling

KAS 是旧版 `PS-Decode` 的收敛版本。它不把贡献写成通用 MLFQ 或纯 LAS，而是强调 decode active set 必须同时满足 attained-service priority、KV residency 和 swap budget 三类约束。
...
> Decode 是 unknown-size iterative serving。KAS 用 generated tokens 作为 attained service，近似 least-attained-service scheduling；同时把 KV residency、KV footprint、free block budget 和 swap budget 作为 admission 约束，避免调度器选择一个理论上紧急但当前激活代价过高的请求。
...
KAS(queues, kv_state, decode_budget):
  ...
  for level from Q0 to Qn:
    for r in ordered(level):
      ...
      if feasible_batch(B, r, decode_budget) and feasible_kv(r, swap_used, decode_budget):
        B.add(r)
...
这些不变量把 KAS 和普通 MLFQ 区分开：它不是单纯按时间片轮转，而是在 unknown output length、KV residency 和 per-iteration feasibility 三者之间做在线折中。
...
KAS 不应承诺严格公平，因为它有意偏向低 attained service 请求以改善短请求 slowdown。但它应提供一个可实验验证的 skip bound：
...
如果 KAS 显著改善短请求但让长请求 slowdown 不受控，顶会 reviewer 会认为算法只是在转移痛点。因此 fairness analysis 必须作为主实验之一，而不是 appendix。
...
## 方法三：PBC：Pressure-Budget Controller

这是最终论文中应该作为 Algorithm 1 呈现的核心算法，而不是附属优化。它负责把 KAS 观测到的 decode-side pressure 转化为 BPS 和 KAS 都能消费的 admission budget，形成闭环。

不建议 claim broad global controller。更稳的做法是定义一个 pressure-to-budget controller：prefill 是否继续积极 dispatch、decode active set 是否允许昂贵 swap-in，都受到当前 queue/KV/swap pressure 的预算约束。
...
> 仅做局部 prefill 或 decode scheduling 仍然可能造成阶段间压力转移；pressure-budget control 可以减少 bridge queue 堆积、decode KV pressure 和 swap pressure，并提升端到端 SLO goodput。
...
它应该被定位为 local pressure-budget coordination，而不是新的大规模控制平面。
...
面向阶段间压力传播的 pressure-budget controller。

目标：

- 将 decode queue、KV block、bridge queue 和 swap pressure 转成结构化 admission budget。
...
这三个算法共同构成一个可发表的系统方法，而不是松散的 heuristic 集合。论文中建议把 PBC 放在最前面呈现，因为它给出了统一的 pressure-to-budget 接口；BPS 和 KAS 是该接口下的两个 phase-specialized execution policies。

最终方法可以概括为：

```text
PhaseServe pressure-budgeted phase scheduling
  = PBC: pressure-budget controller
  + BPS: budgeted prefill shaping
  + KAS: KV-constrained attained-service scheduling
```
...
2. **方法设计**：我们提出 pressure-budgeted phase scheduling，将 decode-side queue/KV/swap pressure 转化为 admission budget，并在该 budget 下将 prefill 建模为 known-size budgeted shaping problem，将 decode 建模为 unknown-size KV-constrained attained-service scheduling problem。
...
- H1：prompt length skew 越高，BPS 对 TTFT p95/p99 的收益越明显。
- H2：在相同 prompt length 分布下，BPS 相比 shortest-prompt-first 应显著降低 long-prompt starvation。
- H3：output length variance 越高，KAS 对 TPOT p95/p99 和 short-request slowdown 的收益越明显。
- H4：在相同 output length 分布下，KV-aware LAS 相比 pure LAS 应减少 swap 次数和 iteration stall。
- H5：decode pressure 越高，PBC 对 SLO goodput、bridge queue 和 swap pressure 的收益越明显。
...
PhaseServe 的贡献不是单个复杂 heuristic，而是识别了 prefill/decode 解耦之后的信息结构差异和压力传播路径，并将一个压力预算控制器和两个预算执行策略映射到 LLM serving runtime：

- PBC: pressure-budget control for pressure propagation across phases；
...
KAS 不承诺严格 max-min fairness，而是使用 `consecutive_skips` 提供 skip-bounded fairness。论文需要报告长请求 tail latency、long-output slowdown、maximum consecutive skips 和 starved admission ratio，而不是只报告平均 TPOT。
...
4. **PhaseServe Design**
   ...
   - KAS：KV-constrained least-attained-service scheduling。
