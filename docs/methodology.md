...
> PhaseServe 是一个 bottleneck-directed runtime scheduling system for phase-disaggregated LLM serving。它证明了，在完成 prefill/decode 静态资源解耦之后，剩余关键瓶颈来自阶段内信息不匹配和阶段间压力传播；通过压力感知的 prefill batching、KV 约束的 decode scheduling 和 bridge-level backpressure admission，可以在不修改模型 kernel 的情况下改善 workload 的瓶颈指标，例如 TTFT queueing、TPOT tail、SLO goodput 或不同长度 bucket 的公平性。
...
2. 它把两个局部调度器和一个 bridge backpressure 机制组织成闭环，避免论文看起来只是两个互不相干的 heuristic。
3. 它能做干净消融：prefill scheduler、decode scheduler、bridge admission 可以分别打开和关闭。

## 最终论文算法呈现

最终论文建议呈现 **3 个算法**，而不是 2 个。

这 3 个算法不是为了声称“所有指标同时提升”，而是分别对应 phase-disaggregated serving 中三类压力：

| 算法 | 解决的压力 | 主要优化指标 | 允许的 tradeoff |
|---|---|---|---|
| Algorithm 1: PS-Prefill | prefill queue pressure 和 prompt-length skew | TTFT median/tail、context queue time、prefill utilization | 在 decode pressure 高时可能牺牲部分 prefill token throughput |
| Algorithm 2: PS-Decode | decode KV pressure 和 unknown output length | TPOT P90/P99、decode queue time、short-output slowdown | 长输出请求平均完成时间可能轻微上升，需要 fairness bound |
| Algorithm 3: PS-Bridge | prefill 到 decode 的压力传播 | SLO goodput、per-GPU goodput、bridge queue、swap pressure | TTFT 或 prefill utilization 可能下降，以避免 decode 过载 |

正文中的呈现方式建议是：

1. 先给出一个统一的 **runtime pressure model**，定义 prefill pressure、decode pressure 和 bridge pressure。
2. 再给出三个算法，每个算法只优化自己对应的瓶颈压力。
3. 最后说明三者如何组成闭环：PS-Decode 产生 decode pressure，PS-Bridge 将压力反馈为 admission/budget，PS-Prefill 根据该反馈调整 batch composition。

不要把论文写成“我们提出了三个独立调度启发式”。更好的叙事是：

> PhaseServe uses three lightweight online algorithms to control how pressure is created, absorbed, and propagated across the disaggregated prefill/decode boundary.

中文表述：

> PhaseServe 用三个轻量在线算法控制解耦式 serving 中压力的产生、吸收和传播。
...
因此，一个好的 disaggregated serving runtime 不应该对两个阶段都使用同一种 FCFS-like 策略，也不应该让 prefill 和 decode 完全各自为政，而应该使用 bottleneck-directed pressure control：

- 对 prefill 使用 pressure-aware cost-compatible batching。
- 对 decode 使用 KV-constrained least-attained-service scheduling。
- 对两个阶段之间的迁移使用 bridge-level backpressure admission。
...
> 在 prefill-decode disaggregated serving 中，阶段间资源隔离之后的剩余瓶颈主要来自阶段内信息不匹配和阶段间压力传播。将 prefill 建模为 known-size constrained batching、将 decode 建模为 unknown-size KV-constrained attained-service scheduling，并通过 bridge-level backpressure 抑制 decode 过载，可以在异构 workload 下改善对应瓶颈指标和 SLO goodput。

这个假设有明确失败条件：如果 workload 的 prompt length 和 output length 都高度同质，或者系统远低于饱和，PhaseServe 的收益应显著下降；如果调度开销或 swap 开销超过收益，则方法不成立。

## 方法一：PS-Prefill：Pressure-Aware Cost-Compatible Batching
...
> 在一个有界候选窗口内，按照 prompt cost compatibility 构造 batch，同时根据 decode pressure 调整 prefill aggressiveness，并用 oldest-request protection 提供 starvation bound。
...
如果 decode pressure 较高，`safety_margin` 增大，候选 batch 的 token budget 和 KV footprint 更保守；如果 decode pressure 较低，`safety_margin` 减小，prefill 可以更积极地填满 batch。
...
```text
PS-Prefill(waiting_queue, decode_pressure):
  C = first K requests from waiting_queue
  p = oldest request in C
  budget = adjust_prefill_budget(decode_pressure)
...
> 在 prompt length 分布偏斜且系统接近饱和时，pressure-aware cost-compatible batching 可以降低 TTFT tail latency；当 decode pressure 升高时，它通过保守 prefill admission 减少阶段间压力传播，同时通过 protected oldest request 保持长 prompt 的 bounded waiting。
...
## 方法二：PS-Decode：KV-Constrained Least-Attained-Service Scheduling
...
> Decode 是 unknown-size iterative serving。PS-Decode 用 generated tokens 作为 attained service，近似 least-attained-service scheduling；同时把 KV residency、KV footprint、free block budget 和 swap budget 作为 admission 约束，避免调度器选择一个理论上紧急但当前激活代价过高的请求。
...
PS-Decode(queues, kv_state):
  ...
  for level from Q0 to Qn:
    for r in ordered(level):
      ...
      if feasible_batch(B, r) and feasible_kv(r, swap_used, decode_pressure):
        B.add(r)
...
> 在输出长度高度可变、短生成和长生成混合的 workload 下，KV-constrained least-attained-service scheduling 可以降低 TPOT tail latency 和短请求 slowdown；同时通过 starvation counter、resident-first admission 和 swap-budget constraint 控制公平性与 swap 开销。
...
## 方法三：PS-Bridge：Bridge-Level Backpressure Admission

这是最终论文中应该保留并强化的第三个算法，而不是附属优化。它负责把 PS-Decode 观测到的压力反馈给 PS-Prefill，形成闭环。

不建议 claim broad global controller。更稳的做法是定义一个 bridge-level backpressure admission policy：prefill 是否继续积极 dispatch，要受到 decode pressure 的反馈影响。
...
4. `swap_in_queue_length`
5. `decode_queue_wait_p95`，可选，第一版可用 queue length 近似。
6. `free_gpu_blocks`
7. `prefill_waiting_age`
...
```text
PS-Bridge(context_state, decode_state):
...
PS-Prefill 不直接读取 decode 内部队列，而是读取 PS-Bridge 生成的 `AdmissionBudget`。这样实现上保持模块边界清楚，论文中也更容易解释。

### 设计不变量

PS-Bridge 应维护三个不变量：

1. **Backpressure invariant**：当 decode pressure 超过 high watermark 时，prefill 不能继续以最大 token budget 积极 dispatch 大 KV footprint batch。
2. **Progress invariant**：即使 decode pressure 高，protected oldest request 仍然可以在 bounded waiting 后被允许通过，避免 prefill starvation。
3. **Hysteresis invariant**：high watermark 和 low watermark 分离，避免 admission budget 在临界压力附近频繁抖动。

### 可发表 claim

Bridge admission 的核心 claim 应该是：

> 仅做局部 prefill 或 decode scheduling 仍然可能造成阶段间压力转移；bridge-level backpressure admission 可以减少 bridge queue 堆积、decode KV pressure 和 swap pressure，并提升端到端 SLO goodput。
...
建议将 PhaseServe 的方法论收敛为 **3 个正文算法**：

### PS-Prefill

面向已知 prompt length 的 pressure-aware cost-compatible batching。
...
- 在 decode pressure 低时保持较高 prefill utilization。
- 在 decode pressure 高时主动降低 KV handoff 压力。

### PS-Decode

面向未知 output length 的 KV-constrained least-attained-service scheduling。

目标：

- 降低 TPOT tail latency。
- 改善短输出请求的排队体验。
- 通过 `consecutive_skips`、resident-first admission 和 swap budget 控制 starvation 与 swap overhead。

### PS-Bridge

面向阶段间压力传播的 bridge-level backpressure admission。
...
这三个算法共同构成一个可发表的系统方法，而不是松散的 heuristic 集合。论文中建议把 PS-Bridge 放在 PS-Prefill 和 PS-Decode 之后呈现，因为它解释了为什么两个局部调度器需要被一个轻量 backpressure loop 连接。

最终方法可以概括为：

```text
PhaseServe runtime pressure control
  = PS-Prefill: pressure-aware cost-compatible batching
  + PS-Decode: KV-constrained attained-service scheduling
  + PS-Bridge: bridge-level backpressure admission
```
...
2. **方法设计**：我们提出 bottleneck-directed runtime pressure control，将 prefill 建模为 known-size pressure-aware batching problem，将 decode 建模为 unknown-size KV-constrained attained-service scheduling problem，并通过 bridge-level backpressure admission 连接两个阶段。

3. **系统实现**：我们在 DistServe 上实现 PhaseServe，替换其 FCFS context scheduler 和 FCFS decode scheduler，并复用现有 block manager 完成 memory-aware admission、swap-aware scheduling 和 decode-pressure feedback。
...
PhaseServe 的贡献不是单个复杂 heuristic，而是识别了 prefill/decode 解耦之后的信息结构差异和压力传播路径，并将三个低成本在线控制算法映射到 LLM serving runtime：

- pressure-aware cost-compatible batching for known-size prefill；
- KV-constrained least-attained-service scheduling for unknown-size decode；
- bridge-level backpressure admission for pressure propagation across phases。

实现是轻量启发式，但设计不是 ad hoc：PS-Prefill 维护 feasibility、oldest protection 和 no artificial waiting 三个不变量；PS-Decode 维护 iteration feasibility、attained-service ordering、resident preference 和 skip-bounded fairness 四个不变量；PS-Bridge 维护 backpressure、progress 和 hysteresis 三个不变量。
...
1. **Introduction**
   ...
   - 给出 motivating example：FCFS 在 skewed prompt、variable output 和 decode pressure 下失败。
...
4. **PhaseServe Design**
   - PS-Prefill：pressure-aware cost-compatible batching。
   - PS-Decode：KV-constrained least-attained-service scheduling。
   - PS-Bridge：bridge-level backpressure admission。
...
- PS-Prefill 的 pressure-aware cost-compatible batch 选择规则和 bounded waiting 规则。
- PS-Decode 的 KV-constrained LAS 队列组织、resident-first tie-breaker 和 skip-based fairness 规则。
- PS-Bridge 的 high/low watermark、admission budget 和 hysteresis 规则。
- 每个机制的消融实验计划。
...
- `PS-Prefill`: pressure-aware cost-compatible prefill batching。
- `PS-Decode`: KV-constrained least-attained-service decode scheduling。
- `PS-Bridge`: bridge-level backpressure admission。

论文副标题或方法概括可以考虑：

> PhaseServe: Bottleneck-Directed Runtime Scheduling for Phase-Disaggregated LLM Serving

或者：

> PhaseServe: Runtime Pressure Control for Prefill-Decode Disaggregated LLM Serving
...
```text
bottleneck-directed runtime pressure control
  = PS-Prefill: pressure-aware cost-compatible batching
  + PS-Decode: KV-constrained attained-service scheduling
  + PS-Bridge: bridge-level backpressure admission
```
