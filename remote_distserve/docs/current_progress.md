...
| `docs/bps_bounded_progress_implementation.md` | BPS bounded-progress 指标与 protected-oldest 修正 | 已新增 |
...
状态：已实现第一版 instrumentation，记录在 `docs/kas_fairness_implementation.md`。

已完成：

- `infeasible_rounds`，区分“资源不可行”和“被策略跳过”。
- `max_consecutive_infeasible`。
- `starved_ready`、`starved_selected`、`starved_admission_ratio`。
- `policy_skipped`。
- infeasible reason breakdown。
- long-output slowdown proxy：`e2e_per_output_token_s`、`decode_per_output_token_s`。

待实现：

- KV-unaware LAS / pure LAS 对照。
- long-output stress 正式验证。

### Step 3：BPS bounded-progress

状态：已实现第一版 instrumentation，记录在 `docs/bps_bounded_progress_implementation.md`。

已完成：

- protected dispatch ratio。
- feasible protected dispatch ratio，排除物理资源不可行的 `protected_blocked` 轮次。
- long-prompt max queue wait。
- long-prompt TTFT p90/p99。
- forced oldest 对吞吐或 TPOT 的 tradeoff。
- protected-oldest 触发但物理不可行时不再绕过选择其它 batch，记录为 `protected_blocked`。

待验证：

- prompt-skew smoke。
- rate `0/4/6/8/10` 下 `fcfs` vs `bps` 的 2-seed 或 5-seed。
- 最长 prompt bucket 的 `context_queue_max` 和 TTFT p99 是否没有失控。
