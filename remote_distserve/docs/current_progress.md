...
PhaseServe 的方法论和代码已收敛到 `typed/regime-aware PBC + BPS + output-tail eligible KAS`，并补上了 ShareGPT 真实 trace 暴露出的 `conflict-first PBC + bridge completion drain`：PBC 负责 typed pressure-to-budget、regime ownership 和 first/decode 冲突仲裁，BPS 作为 first-token/prefill 侧 TTFT owner，KAS 作为 decode 侧 TPOT owner 和 bridge 高压下的 KV release owner。最新 OPT-13B + ShareGPT 128-request seed0/seed1 已在 global rate `1.5` 同时改善 TTFT 与 TPOT 的多个分位和 SLO，seed0 goodput 也提升；下一步是扩 seed/rate 和正式画图区间收窄。
...
| `docs/stage2_arbitration_fix.md` | Stage 4A 反推的 PBC/BPS/KAS 仲裁修复、验证和下一轮 Stage 4A 入口 | 已完成 |
...
| `docs/stage4e_trace_baseline_calibration.md` | 真实 ShareGPT/LongBench trace 的 E1 baseline calibration 记录，包含数据集状态、OPT-13B+ShareGPT、LLaMA2-13B+LongBench 和 LLaMA2-13B+ShareGPT baseline pressure window | 已完成 seed0 初始窗口 |
| `docs/stage4f_e2_main_end_to_end.md` | E2 主端到端实验进展，记录 PhaseServe full 在 E1 固定窗口上与 DistServe/FCFS 的对比 | 已完成 seed0 初始结果 |
| `docs/stage4_w0_smoke_triage.md` | OPT-13B W0 smoke 结果、验收、风险和 LLaMA2-13B 阻塞记录 | OPT-13B 已完成，LLaMA2-13B 待授权 |
| `docs/stage4_w2_ttft_calibration.md` | OPT-13B prompt-skew 高压 TTFT 校准，定位 BPS 有效区间、full Phase 仲裁冲突、typed PBC 修复和 KAS gating 试验 | 已完成 seed 0 校准与修复收口 |
| `docs/stage4_decode_regime_validation.md` | regime-aware PBC 代码对齐后的 OPT-13B decode-heavy seed0/rate2 性能验证 | 已完成最小验证 |
| `docs/stage4a_prompt_skew_metric_audit.md` | OPT-13B prompt-skew seed0/1 指标审计，筛选 TTFT/TPOT candidate windows 并判断是否需要回到 Stage 2 | 已完成，结论是先修 full Phase 仲裁 |
| `docs/stage4b_scheduler_repair.md` | Stage 4A 之后的 PBC/BPS/KAS 仲裁循环修复，记录 short-output gate、long-output full-KAS gate 和最新 seed0 验证 | 已完成初步收口 |
| `docs/stage4c_mixed_regime_end_to_end.md` | OPT-13B mixed-regime full Phase vs FCFS 端到端验证 | 已完成 |
| `docs/stage4c_mechanism_audit.md` | Stage 4C mechanism audit，解释 mixed-regime 中 PBC/BPS/KAS 的机制链路 | 已完成 |
| `docs/stage4c_mixed_regime_ablation.md` | Stage 4C mixed-regime 组件消融，比较 `fcfs/bps/kas/bps_kas/phase` | 已完成 |
| `docs/stage4c_tpot_exploration.md` | Stage 4C TPOT rate fine sweep 与 decode-heavy stress，筛选 TPOT 辅助窗口 | 已完成 |
| `docs/stage4d_13b_mixed_wide_summary.md` | 最新双模型 13B mixed-wide 端到端汇总，包含新 SLO、OPT/LLaMA rate 表和论文 claim 边界 | 已完成 |
| `docs/stage4d_tpot_highrate_pilot.md` | TPOT high-rate pilot 与确认矩阵，检查 per-GPU `14/16` 是否可纳入 TPOT 主图 | 已完成 |
| `docs/stage4h_opt_sharegpt_phase512_repair.md` | OPT-13B + ShareGPT 上修复 full Phase TTFT/TPOT tradeoff，记录 long-output full KAS threshold 从 `192` 调整到 `512` 的诊断、代码改动和 seed0/1 验证 | 已完成 |
| `docs/stage4i_tpot_diagnostic_and_kas_bridge_repair.md` | OPT-13B + ShareGPT 128-request 诊断，修复 bridge/unaccepted pressure transfer，记录 PBC conflict-first 和 KAS bridge completion drain 的 spot validation | 已完成 seed0/seed1 rate1.5 spot |
| `docs/stage4j_opt_sharegpt_final128_pergpu1to5.md` | 最终默认代码在 OPT-13B + ShareGPT 128-request、per-GPU `1-5` 上的两 seed 复测，判断 Phase512 旧结果能否直接沿用 | 已完成 |
...
3. **KAS 初版**
   ...
   - 已关闭失败分支默认值：`PHASESERVE_KAS_BRIDGE_HOL_BYPASS=0`，`PHASESERVE_KAS_BRIDGE_SHORT_OUTPUT_FASTLANE=0`。
...
6. **OPT-13B W2 TTFT high-rate calibration 已完成**
   ...
   - 详见 `docs/stage4_w2_ttft_calibration.md`。
...
7. **OPT-13B Stage 4A prompt-skew metric audit 已完成**
   ...
   - 详见 `docs/stage4a_prompt_skew_metric_audit.md`。

8. **Stage 2 PBC/BPS/KAS 仲裁修复已完成**
   - 修复文档：`docs/stage2_arbitration_fix.md`。
...
当前处于 Stage 4D 历史结果收口之后的实验协议重置入口。根据对 `related_papers/` 中 DistServe、WindServe 和 ShuffleInfer 的重新审阅，后续正式实验以 `docs/experiment_protocol.md` 为准：主端到端实验回到 `ShareGPT` / `LongBench` 真实长度分布，先做 DistServe-only baseline calibration，再在相同 pressure window 下比较 PhaseServe。既有 mixed-wide / cross-skew 结果保留为 exploratory 和机制线索，不再作为唯一主实验设计来源。

下一步不应继续盲目搜索 workload 或 rate，而应按下面顺序推进：

1. E0 收口：确认 `docs/experiment_protocol.md`、`docs/benchmarking.md` 和当前 benchmark 指标口径一致。
2. E1 baseline calibration：检查或补齐 ShareGPT/LongBench trace loader，然后只跑 DistServe/FCFS，寻找 P0 `OPT-13B + ShareGPT`、P0 `LLaMA2-13B + LongBench` 和 P1 `LLaMA2-13B + ShareGPT` 的 baseline pressure window。三组 seed0 初始窗口均已完成，详见 `docs/stage4e_trace_baseline_calibration.md`。`OPT-13B + LongBench` 只作为 P2 可选项，因为 OPT 的 2K context 限制会引入截断解释成本。
3. E2 main end-to-end：在 E1 固定窗口上跑 PhaseServe full，对比 DistServe/FCFS。三组真实 setting 的 seed0 初始结果均已完成，详见 `docs/stage4f_e2_main_end_to_end.md`。
4. E2B broad rate stress：补充 per-GPU `0-10 req/s` 范围的宽 rate 扫描，用于确认低压、有效压力和过载边界。1P1D 下脚本全局 `RATES` 需要扫到 `20 req/s`，详见 `docs/stage4g_broad_rate_stress.md`。
5. E3 消融：在代表性 pressure rate 上跑 `w/o PBC`、`w/o BPS`、`w/o KAS` 和 full PhaseServe。
6. E4 ShuffleInfer-style baseline：构造 LPLD/LPHD/HPLD/HPHD/Mixed，加入 same-engine length-aware prefill/decode baseline。
7. E5 机制分析：抽取 queue breakdown、pressure-to-budget timeline、bucket tail 和 KV/swap feasibility 证据。
...
1. OPT-13B W0 policy smoke 已通过，详见 `docs/stage4_w0_smoke_triage.md`。
