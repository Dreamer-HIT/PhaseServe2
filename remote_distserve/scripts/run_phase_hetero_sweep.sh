...
FIXED_DATASET_METADATA=${FIXED_DATASET_METADATA:-}
...
for seed in ${SEEDS}; do
  for rate in ${RATES}; do
    ...
    dataset="${FIXED_DATASET:-${SWEEP_ROOT}/datasets/hetero_seed_${seed}.marshal}"
    dataset_metadata="${FIXED_DATASET_METADATA:-${SWEEP_ROOT}/datasets/hetero_seed_${seed}.metadata.json}"
...
"${PYTHON}" benchmarks/phase_slo_grid.py \
  "${SWEEP_ROOT}" \
  --grid "${SLO_GRID}" \
  --output-prefix "${SWEEP_ROOT}/slo_grid"
